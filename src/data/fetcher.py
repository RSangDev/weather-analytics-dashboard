...
class WeatherDataFetcher:
    ...
    def fetch_city_weather(
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                logger.warning(
                    f"Attempt {attempt + 1}/{self.retry_attempts} failed for {city_name}: {e}"
                )
                if attempt < self.retry_attempts - 1:
                    time.sleep(2 ** attempt)  # Exponential backoff
                else:
                    logger.error(f"Failed to fetch data for {city_name} after all attempts")
                    return None
    ...
    def fetch_all_cities(self, cities: List[Dict], forecast_days: int = 7) -> List[Dict]:
        """
