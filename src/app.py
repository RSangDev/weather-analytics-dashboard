...
def consolidate_alerts(alerts):
    ...
    for (city, alert_type, date), group in grouped:
        ...
        if alert_type in ['high_temperature', 'high_wind', 'heavy_precipitation']:
            ...
            if count > 1:
                message = f"{group['message'].iloc[0]} (pico: {max_value:.1f}, {count}x no dia)"
            else:
    ...
    consolidated_df = consolidated_df.sort_values(['date', 'severity'], ascending=[False, True])
...
def create_weather_metrics_dashboard(daily_df, selected_city):
    ...
    colors = ['lightblue' if p < 10 else 'blue' if p < 30 else 'darkblue' 
...
def main():
    ...
    # Header
    st.markdown('<div class="main-header">🌤️ Weather Analytics Dashboard</div>', 
                unsafe_allow_html=True)
    ...
    **Fonte:** Open-Meteo API
    ...
    with col1:
        st.metric(
            "🏙️ Cidades",
            stats['total_cities'],
            help="Número de cidades monitoradas"
        )
    ...
    with col5:
        anomaly_delta = "normal" if stats['anomalies_detected'] == 0 else "attention"
        st.metric(
            "⚠️ Anomalias",
    ...
    with col6:
        alert_delta = "normal" if len(consolidated_alerts) == 0 else "attention"
        st.metric(
            "🚨 Alertas",
            len(consolidated_alerts),
            help="Alertas meteorológicos ativos"
        )
    ...
    if consolidated_alerts:
        ...
        with col_list:
            ...
            if selected_alert_type != "Todos":
                filtered_alerts = [a for a in consolidated_alerts if a['type'] == selected_alert_type]
            else:
    ...
    with tab2:
        ...
        st.info("""
        📝 **Sobre a Média Móvel:**  
        A linha azul escura representa a média móvel de 3 horas, que suaviza as flutuações
    ...
    with tab3:
        ...
        metrics_dashboard = create_weather_metrics_dashboard(daily_df, selected_city_metrics)
    ...
    with tab4:
        ...
        with col_filter2:
            show_anomalies_only = st.checkbox("Mostrar apenas anomalias", value=False)
    ...
    st.markdown("""
        <div style='text-align: center; color: #666; padding: 2rem 0;'>
            <strong>Data Source:</strong> <a href="https://open-meteo.com" target="_blank">Open-Meteo API</a><br>
