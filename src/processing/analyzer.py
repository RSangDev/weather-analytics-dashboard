...
class WeatherDataProcessor:
    ...
    def calculate_moving_averages(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for city in df["city"].unique():
            ...
            df.loc[city_mask, "temp_ma"] = (
                df.loc[city_mask, "temperature_2m"]
    ...
    def detect_anomalies(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for city in df["city"].unique():
            ...
            if std_temp > 0:
                anomaly_mask = np.abs(city_temps - mean_temp) > (
                    self.anomaly_threshold * std_temp
                )
    ...
    def generate_alerts(self, df: pd.DataFrame) -> List[Dict]:
        ...
        # Temperature alerts
        high_temp_mask = (
            df["temperature_2m"] > self.alert_config["temperature"]["high_threshold"]
        )
        low_temp_mask = (
            df["temperature_2m"] < self.alert_config["temperature"]["low_threshold"]
        )
        ...
        # Wind speed alerts
        high_wind_mask = (
            df["wind_speed_10m"] > self.alert_config["wind_speed"]["high_threshold"]
        )
        ...
        high_precip_mask = (
            df["precipitation"] > self.alert_config["precipitation"]["high_threshold"]
        )
    ...
    def get_city_rankings(self, daily_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        rankings = {
            "hottest": latest.nlargest(10, "temp_max")[
                ["city", "temp_max"]
            ].reset_index(drop=True),
            "coldest": latest.nsmallest(10, "temp_min")[
                ["city", "temp_min"]
            ].reset_index(drop=True),
            "rainiest": latest.nlargest(10, "precipitation_total")[
                ["city", "precipitation_total"]
            ].reset_index(drop=True),
            "windiest": latest.nlargest(10, "wind_max")[
                ["city", "wind_max"]
            ].reset_index(drop=True),
            "most_humid": latest.nlargest(10, "humidity_mean")[
                ["city", "humidity_mean"]
            ].reset_index(drop=True),
        }
    ...
    def detect_weather_patterns(self, df: pd.DataFrame) -> Dict:
        ...
        """
        patterns = {"heat_waves": [], "cold_fronts": [], "heavy_rain_events": []}
        ...
        for city in df["city"].unique():
            ...
            city_df["heat_group"] = (
                city_df["heat_wave"] != city_df["heat_wave"].shift()
            ).cumsum()
                ...
                patterns["heat_waves"].append(
                    {
                        ...
                        "max_temp": city_df[city_df["heat_wave"]][
                            "temperature_2m"
                        ].max(),
                    }
                )
